  ...
  <main class="max-w-5xl mx-auto px-6 pb-12 pt-28 md:pt-36">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
      <router-link 
        ...
        :key="post.slug" 
        :to="'/blog/' + post.slug"
        class="group relative p-6 md:p-8 rounded-3xl bg-white/60 dark:bg-white/[0.02] border border-black/[0.06] dark:border-white/[0.05] hover:bg-white/80 dark:hover:bg-white/[0.04] hover:border-primary-500/30 transition-all duration-300 backdrop-blur-md overflow-hidden hover:-translate-y-1.5 flex flex-col justify-between shadow-sm dark:shadow-none"
      >
        ...
        <div>
          ...
          <div class="flex items-center gap-3 mb-4">
            <span class="px-3 py-1 rounded-full text-xs font-semibold bg-primary-500/10 text-primary-600 dark:text-primary-400 border border-primary-500/20">
              {{ post.tag }}
            </span>
          ...
          <!-- Title -->
          <h3 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-3 group-hover:text-primary-600 dark:group-hover:text-primary-300 transition-colors tracking-tight leading-snug">
            {{ post.title }}
          </h3>

          <!-- Summary -->
          <p class="text-gray-500 dark:text-gray-400 text-sm leading-relaxed mb-6 font-light">
            {{ post.summary }}
          </p>
        </div>

        <!-- Footer Meta (Date and Link) -->
        <div class="flex items-center justify-between mt-auto pt-4 border-t border-black/[0.05] dark:border-white/[0.04]">
          <span class="text-xs text-gray-400 dark:text-gray-500 font-medium">
            {{ post.date }}
          </span>
          <span class="text-xs font-bold text-primary-600 dark:text-primary-400 group-hover:text-primary-500 dark:group-hover:text-primary-300 flex items-center gap-1 transition-colors">
            Baca Selengkapnya
